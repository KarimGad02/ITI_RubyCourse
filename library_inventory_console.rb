...
class ShelfInventory
  ...
  def add_book(title, author, isbn)
    ...
    if matched_book
      ...
      @books << Volume.new(title, author, isbn)
  ...
  def save_data
    File.open(DATA_FILE, 'w') do |file|
      @books.each { |book| file.write(book.as_row) }
    end
  end
  ...
  def load_data
    ...
    File.readlines(DATA_FILE).each do |line|
      ...
      @books << Volume.new(data[0], data[1], data[2], count)
    end
  end
end

# Backward-compatible aliases for older task files that still reference these names.
Book = Volume unless defined?(Book)
BookInventory = ShelfInventory unless defined?(BookInventory)

def start_inventory_console
  catalog = ShelfInventory.new
  keep_running = true

  while keep_running
    puts "\n=== Karim's Library Inventory ==="
    ...
    selected_option = gets.chomp.to_s.strip
    ...
    when "5"
      ...
      search_type = gets.chomp.to_s.strip

      if ["1", "2", "3"].include?(search_type)
        print "Enter search query: "
        query = gets.chomp.to_s.strip
...
start_inventory_console if __FILE__ == $PROGRAM_NAME